...
properties
    ...
    source_timeseries; % HDF5 link to TimesSeries that this data was calculated from. Metadata about electrodes and their position can be read from that ElectricalSeries so it is not necessary to store that information here
end
...
methods
    function obj = DecompositionSeries(varargin)
        ...
        varargin = [{'comments' 'no comments' 'data_conversion' types.util.correctType(1.0, 'float32') 'data_resolution' types.util.correctType(0.0, 'float32') 'description' 'no description' 'help' 'General time series object'} varargin];
